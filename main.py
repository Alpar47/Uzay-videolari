...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Merhaba baba! En uzun ve en kaliteli uzay videolarını seçmeye hazırım. Konuyu yaz yeterli."
    )

async def search_videos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.message.text
    await update.message.reply_text(f"'{query}' için en uzun uzay videosu, senaryo ve ses hazırlanıyor baba...")
    
    headers = {"Authorization": PEXELS_API_KEY}
    # 20 tane video istiyoruz
    ...
    if response.status_code == 200:
        ...
        if videos:
            # Gelen videoların içinden süresi EN UZUN olanı otomatik buluyoruz!
            longest_video = max(videos, key=lambda v: v.get("duration", 0))
            video_files = longest_video.get("video_files", [])
