...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Merhaba baba! En uzun uzay videolarını getirmek için buradayım. Konuyu yaz yeterli."
    )

async def search_videos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.message.text
    await update.message.reply_text(f"'{query}' için video, senaryo ve ses hazırlanıyor baba...")
    ...
    if response.status_code == 200:
        ...
        if videos:
            # En uzun videoyu güvenli bir şekilde seçiyoruz
            longest_video = videos[0]
            max_duration = 0
            for v in videos:
